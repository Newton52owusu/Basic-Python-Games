...
def dice(d):
    ...
    while pygame.time.get_ticks() - time_clock & lt; 1000:
        game_layout_display.blit(d, (300, 500))
    pygame.display.update()


# for mute and unmute
def button2(gt, t, xm, ym, x, y, wid, hei, int, after, fast):
    # mouse position
    mouse = pygame.mouse.get_pos()
    click = pygame.mouse.get_pressed()
    if x + wid & gt and xm & gt and x and y + hei & gt and ym & gt and y:
...
# Buttons for playing:
def button1(gt, xm, t, ym, x, y, wid, hei, int, after, fast):
    # mouse position
    mouse = pygame.mouse.get_pos()
    click = pygame.mouse.get_pressed()
    if x + wid & gt and xm & gt and x and y + hei & gt and ym & gt and y:
...
def turn(sc, lefted, section):
    ...
    sc -= d
    time_clock = pygame.time.get_ticks()
    while pygame.time.get_ticks() - time_clock & lt; 1500:
...
# Buttons:
def button(gt, t, xm, ym, x, y, wid, hei, int, after, fast, best):
    if x + wid & gt and xm & gt and x and y + hei & gt and ym & gt and y:
        pygame.draw.rect(game_layout_display, after, [x - 2.5, y - 2.5, wid + 5, hei + 5])
    if pygame.mouse.get_pressed() == (1, 0, 0):
        if best == 1:
...
def introduction():
    time_clock = pygame.time.get_ticks()
    while pygame.time.get_ticks() - time_clock & lt; 2500:
        game_layout_display.blit(initial_background, (0, 0))
    pygame.display.update()
    while True:
        time_clock = pygame.time.get_ticks()
    while pygame.time.get_ticks() - time_clock & lt; 500:
...
if 5 & gt; best & gt; 1 or best == 21:
    game_layout_display.blit(yellow_c, (xcy, ycy))

if 5 & gt; best & gt; 2 or best == 21:
...
if gamer1score == 100:
    ...
    while pygame.time.get_ticks() - time & lt; 2000:
...
if True:
    if tips == 2:
        ...
        if not six:
            tips += 1
            if best & lt; 3 or best == 21:
                tips = 1

if gamer2score == 100:
    time_clock = pygame.time.get_ticks()
    while pygame.time.get_ticks() - time_clock & lt; 2000:
