...
# make a model with a different color on each face
...
def rotate_side(normal, direction=1, speed=1):
    ...
    if speed:
        ...
        invoke(setattr(e, 'world_parent', scene) for e in cubes)
...
def randomize():
    faces = (Vec3(1, 0, 0), Vec3(-1, 0, 0), Vec3(0, 1, 0). Vec3(0, -1, 0), Vec3(0, 0, 1), Vec3(0, 0, -1))
    for i in range(20):
